...
3. This step will create few folders and files. First, it will create three *txt* files containing the names of molecules that were randomly sampled. Note that the name of these files will always start with train, valid and test, even if they will all correspond to training set augmentation after iteration 1. Then, a *morgan* folder will be generated, containing three *csv* files with calculated Morgan fingerprints for the sampled molecules. Finally, it will generate three *smi* files in a *smile* folder, with the SMILES of the sampled molecules, to be used as input for docking (phase 2). 
